...
process = subprocess.Popen(
    f"celery -A celery_worker worker --loglevel=DEBUG --pool=prefork --concurrency=2 --queues=high_priority,default --max-tasks-per-child=1000 --max-memory-per-child=512000 --hostname={worker_name_prefix + generate_random_string(8)} 2>&1",
    shell=True,
